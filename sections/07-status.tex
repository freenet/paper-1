...
Neighbour host-advertisement mesh & Deployed & Update fan-out and terminus lookup \\
...
\paragraph{Durable storage of cold state.} Contract state persists only as long as some peer near $\ell(\contract)$ keeps it, and each peer holds contracts in a byte-budgeted hosted set whose size defaults to a fraction of the host's own memory. When a peer runs over budget it evicts the contract that the fewest parties depend on: candidates are ordered by subscriber count --- local client subscriptions ahead of downstream subscribers --- with ties broken by least-recent genuine \textsf{GET} or \textsf{PUT} access. Subscriber count is an ordering, not a hard pin: if nothing less-depended-upon is eligible, even a subscribed contract is shed and its subscription torn down. Eviction is not a mere cache miss, either --- it reclaims the contract's persisted state, parameters, and code blob from disk. A contract that goes \emph{cold} (nobody reading it, nobody subscribed) is therefore the first victim on every peer that holds it. Nothing in the platform re-replicates a contract to maintain a target replica count: when a hosting peer evicts, no other peer is recruited to take its place, and a loss is repaired only by a fresh \textsf{PUT} from someone who still holds the state. An unpopular contract's state can therefore be evicted from every peer that hosts it and become unrecoverable. The platform makes \emph{no durability guarantee} for cold state; persistence is emergent from access patterns and the replication factor, not a promise. This is the same property the original Freenet had, and it bears on precisely the long-lived applications this paper foregrounds --- a dormant collaborative document, an inbox nobody has polled, an identity-directory entry for an inactive user all hold state that may sit cold for long periods. A mechanism that maintains a minimum replica count independently of demand (active repair, incentivised pinning, or explicit archival contracts) is open work.
