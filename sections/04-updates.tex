...
A host-advertisement mesh provides a second propagation path. A peer that begins hosting a contract announces it to its neighbors, independently of subscriptions. Update targets are the deduplicated union of subscribers and neighboring hosts known through this mesh. Subscriptions are explicit and lease-bound, while hosting follows cache admission and eviction. The mesh prevents an unsubscribed host from silently serving stale state.
