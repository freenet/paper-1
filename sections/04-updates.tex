...
\begin{enumerate}[leftmargin=1.4em, itemsep=2pt, topsep=2pt]
  \item a \emph{state space} $\statespace_\contract \subseteq \{0,1\}^*$ of byte strings, with a \emph{validity predicate} $\valid_\contract : \{0,1\}^* \to \{\bot, \top\}$;
...
Let $u_1, \ldots, u_n \in \statespace_\contract$ be a finite multiset of valid updates and let $\sigma_0 \in \statespace_\contract$ be a starting state. For any permutation $\pi$,
\[
...
\noindent The argument is immediate from associativity and commutativity. Idempotence further makes duplicate delivery harmless. Together these properties are the sufficient conditions for the Strong Eventual Consistency theorem of Shapiro et al.\ \citep{shapiro2011crdt}: peers that have delivered the same set of updates converge regardless of order or repetition.
...
\subsection{The Generalization over Fixed-Lattice CRDTs}

A conventional CRDT runtime offers a closed catalogue of lattices. Each application chooses one. The contract interface inverts this: the application supplies the lattice. Two consequences follow.

First, the platform cannot statically verify that a contract's $\merge_\contract$ is associative, commutative, and idempotent. A contract that violates these properties will exhibit non-converging replicas. Because the contract is content-addressed, however, any deviation is attributable to a specific module; a contract that does not converge is bug-compatible with the rest of the network only insofar as users choose to deploy it.

Second, an application can choose merge semantics tuned to its workload. The consistency requirements of a chat room, a reputation contract, and a public file feed are very different, and bundling all of them into a single lattice family would either force one to pay another's cost or leave several of them unimplementable.
...
The protocol is symmetric: $B$ runs the same exchange in the other direction. In practice the two directions are interleaved over a single connection. Bandwidth is bounded by $|s_A| + |s_B| + |\delta_A| + |\delta_B|$, each of which is application-defined.

\subsection{Malformed Summaries and Malicious Deltas}

The honest-but-stale case is the easy one. Two cases require more care.

\paragraph{Approximate summaries.} A summary type such as a Bloom filter has false positives: $B$ may believe $A$ already has a record that in fact $A$ does not. In the bare protocol, the record is then not included in the delta and the two replicas remain divergent. Contracts that use approximate summaries must therefore either fall back to a more precise second round when divergence is suspected, or accept the false-positive rate as part of their convergence story (relying on later rounds to repair). The platform offers no built-in fallback; the contract designs the round structure.
