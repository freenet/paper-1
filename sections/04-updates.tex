...
The tree is not the only propagation path. A peer that comes to host a contract announces the fact to its neighbours, so peers near $\ell(\contract)$ maintain a mesh of who-hosts-what independent of who has subscribed. An update's broadcast targets are the union of the subscriber set and the neighbouring hosts known through that mesh, deduplicated. The two mechanisms are deliberately kept separate because they have different lifecycles: a subscription is explicit and lease-bound, whereas hosting follows cache admission and eviction. The mesh is what keeps a nearby host that never subscribed from silently serving stale state.

Because $\merge_\contract$ is commutative, simultaneous updates injected at multiple points of the tree do not require coordination. They propagate along the edges, meet at internal nodes, and merge; the order in which different updates arrive at a given node does not affect its final state. Subscriptions are lease-based: in the deployed configuration a lease lasts 8 minutes and a background task renews it every 2 minutes --- but only while the contract is still \emph{in use} at that peer, meaning it has a local client subscription or a registered downstream subscriber. A hosting peer with no remaining interest simply stops renewing; its lease lapses and the branch collapses inward toward $\ell(\contract)$. A subscriber whose lease expires must re-subscribe. This interest-gating is what makes the tree self-healing and keeps the live subscription set proportional to active demand rather than to accumulated cache: orphaned branches die out within a small multiple of the lease period.
