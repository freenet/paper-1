...
\paragraph{Nearest-neighbor lattice.} Kleinberg's bound also assumes a short-range base lattice. Each production peer therefore seeks and retains its nearest connected successor and predecessor on the ring. Maintenance \textsf{CONNECT}s routed toward the peer's own location discover closer candidates on either side; a strictly closer candidate can be admitted even at capacity, and ordinary score-based pruning does not discard the closest live edge on either side. The other connections remain available for the long-range distribution described above. The lattice is active when the configured connection budget is at least the deployed minimum of 25; sparse simulations with smaller budgets leave it disabled so that two reserved short links do not consume most of the topology.

\paragraph{Bounded fan-out.} Admission control normally bounds the connection count between configurable minimum and maximum sizes (deployed defaults 25 and 200). Tightening both lattice sides may transiently exceed the maximum by at most two connections; maintenance then removes non-lattice links to return to the configured bound.
...
The local peer maintains a rolling history of routing events of the form $(q,d,\tau,b,f)$, where $q$ is the selected neighbor, $d$ is its ring distance to the request's target, $\tau$ is the observed time-to-first-byte, $b$ is the observed transfer rate, and $f$ is a failure indicator. From this history the router fits three distance-indexed baseline estimators:
\[
...
The choice of isotonic regression encodes a structural prior we are willing to commit to (latency and failure are non-decreasing in distance-to-target, on average) without committing to a parametric family for the shape of the curve. The pool-adjacent-violators algorithm fits each curve in time logarithmic in history size; old events age out so the model tracks current conditions. Per-peer adjustments account for stable differences among neighbors that the global distance curve does not capture.

The router blends these isotonic baselines with predictions from Renegade \citep{renegade}, a k-nearest-neighbor model whose features identify the peer, contract location, distance, and observation time. Renegade predicts the same three outcomes rather than a residual correction. For each outcome $y$ the combined prediction is
\[
  \widehat{y} = (1-w)\widehat{y}_{\mathrm{iso}} + w\widehat{y}_{\mathrm{knn}},
  \qquad 0 \leq w \leq \tfrac{1}{2},
\]
where $w$ grows as that prediction stage accumulates observations. The isotonic component supplies the monotone distance prior and the k-nearest-neighbor component captures interactions that distance alone cannot express, such as a peer that performs normally for most contracts but selectively drops requests for one region of the key space.

Because the models consume end-to-end routing outcomes, their predictions combine the neighbor's link quality (RTT, bandwidth, and reliability) with the quality of onward paths through that neighbor. Onward-path quality changes more rapidly under churn because it depends on peers beyond the immediate link. The rolling history removes stale observations as new events re-score the neighbor.

Given a target $t$ for a new request, the router selects the neighbor that minimizes an expected-cost score combining the three blended predictions. A neighbor with too little history falls back to greedy-by-distance until enough events have accumulated. The isotonic predictions use linear interpolation between fitted points and extrapolate linearly beyond the training-data domain.
...
\paragraph{\textsf{PUT}.} A \textsf{PUT} carries a contract $\contract$ and an initial state $\sigma_0$. The originator validates and stores the state, computes its contract-defined summary, and routes a probe toward $\ell(\contract)$. An intermediate peer that does not already hold the contract forwards the probe without storing the state. If the probe finds a fresh holder, that peer returns its summary; the originator computes the corresponding delta and sends it to the holder, which merges it through the ordinary \textsf{UPDATE} path and co-host fan-out.

If the probe finds no fresh holder, fails, or cannot use the summary-first protocol with the next peer, the operation falls back to the full-state \textsf{PUT} path. On that path, routed peers validate and host the state as before. Both paths remain subject to the hosting budget, and there is no separate cache tier beneath hosting. A peer that begins hosting announces the fact to its neighbors (\cref{sec:updates}).
...
For each \textsf{GET} and \textsf{PUT} transaction we compute a \emph{path length}: the number of distinct peers seen emitting the corresponding \textsf{*\_request} event for that transaction id, which is the number of peers visited (hops $+ 1$). Statistics over operations whose telemetry reached completion in the window:\footnote{In this May 2026 snapshot, \textsf{UPDATE} (median path length 2, $n = 386$) and \textsf{SUBSCRIBE} (median 1, $n = 5{,}359$) propagated primarily through pre-existing subscription paths rather than fresh greedy routes, so their path lengths reflected the distance from the subscriber to the nearest hosting peer rather than routing-layer hop counts. They are excluded from the table here.}
