...
Because the estimators are indexed by distance-to-target, each fitted curve folds together two quantities that are worth separating conceptually: the neighbor's own link quality (its RTT, bandwidth, and reliability) and the quality of the sub-network reachable \emph{through} that neighbor toward targets at that distance. The second component shifts under churn faster than the first, since it depends on peers beyond the immediate neighbor rather than on the neighbor's own link. The aging window is what keeps each curve tracking the current composite rather than a topology that has since moved; a neighbor whose onward paths have degraded is re-scored as its recent events accumulate, not held at a stale estimate.
...
A route that terminates without finding a host does not immediately report a miss. The terminus consults the host advertisements its neighbours have announced (\cref{sec:updates}) and, if one of them holds the contract, forwards there. Findability is thus routing \emph{plus} advertisement, not routing alone: greedy forwarding gets a request into the right neighbourhood of the ring, and the advertisement mesh covers the last step when the specific peer the metric selected is not itself a host. \textsf{SUBSCRIBE} terminates the same way.
