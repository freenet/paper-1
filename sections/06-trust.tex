...
\emph{Topology construction.} Gap-targeting (\cref{sec:routing}) assumes that neighbor locations are distributed around the ring. A cluster of co-located peers appears as a zero-width spike to the largest-log-distance-gap computation. The nearest-neighbor lattice makes the closest successor and predecessor especially important: an adversary able to grind locations on both sides of a target can occupy those predictable short-range slots and attempt a local eclipse, although ordinary liveness eviction still removes failed links. \emph{Replication.} Peers near a contract location are intended to protect against a single peer serving stale or doctored state. Co-located peers may instead share an operator and failure domain, reducing the independence of those replicas. \emph{Sybil bootstrapping.} Application-layer defenses depend on honest peers being geographically diverse near critical locations, which shared prefixes make less likely.
