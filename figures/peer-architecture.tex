...
\begin{tikzpicture}[
  ...
  % --- Browser on the left ---
  \node[ui] (ui) at (-0.4,0) {UI bundle\\\scriptsize(HTML+JS+WASM)};
  \node[label, anchor=south] at (ui.north) {browser};

  % --- Freenet Core in the middle ---
  \node[core] (core) at (5.8,0) {};
  ...
  \node[wasm] (c1) at (4.8,-0.1)  {contract};
  \node[wasm] (d1) at (6.8,-0.1) {delegate};
  ...
  \node[netcloud, right=18mm of core.east] (net) {rest of network};

  % --- Wires ---
  \draw[arr] (ui.east) --
       node[label, fill=white, inner sep=1.5pt, midway, above=3pt]
       {WebSocket} (core.west);
  \coordinate (udpturn) at ([xshift=10mm] t.east);
  \draw[arr] (t.east) --
       node[label, fill=white, inner sep=1pt, midway, below=2pt] {UDP}
       (udpturn) |- (net.west);
